...
public class GiangVienApiController {
    ...
    public ResponseEntity<GetLop> getLops(@PathVariable("idGiangVien") Integer idGiangVien, @PageableDefault(size = 10, page = 0, direction = Sort.Direction.ASC) Pageable pageable){
        return ResponseEntity.ok(lopService.getLop(pageable,idGiangVien));
    }
    @GetMapping("/{idGiangVien}/danhsachlophocphan")
    public ResponseEntity<GetLopHocPhan> getLopHocPhans(@PathVariable("idGiangVien") Integer idGiangVien, @PageableDefault(size = 10, page = 0, direction = Sort.Direction.ASC) Pageable pageable){
